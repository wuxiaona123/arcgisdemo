...
import Arcgismaptth from '../Arcgismap3'
import One from './One';
                    ...
                    <div className="leftLayer" style={this.state.tigclose?{transform:'translateX(-100%)'}:null}>
                        ...
                        <div className="rightshow" style={{ display:this.state.display }}>
                            ...
                            <div className="itemshow">
                                <div className="Itemshow">
                                    {this.state.showitem==='pandect'?<One />:null}
                                    {this.state.showitem==='monitor'?'二':null}
                                    {this.state.showitem==='gis'?'三':null}
                                    {this.state.showitem==='prodect'?'四':null}
                                </div>
                            </div>
                        </div>
                        <div className="basedataShow" style={{ display:this.state.displaybasedata }}>
                            ...
                            {this.state.leftShow === 'basedata' ?'一': null}
                            {this.state.leftShow === 'monitorequip' ? '二' : null}
                            {this.state.leftShow === 'remotesensing' ?'三' : null}
                            {this.state.leftShow === 'spatialanalyze' ?'四': null}
                            {this.state.leftShow === 'layermanagement' ?'五' : null}
                            {this.state.leftShow === 'miningnavigation' ?'六' : null}
                        </div>
                    </div>
                </div>
            </div>
